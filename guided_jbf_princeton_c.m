function [mask_gbf, weight_o, g_t, weight_s]= guided_jbf_princeton_c(mask, I, is_ime, count_i, weight, ...
    sigma, win_width, thres, weight_sum, rectIn)
    global debug_mode;
    I = double(I);
    [H, W]  =size(mask);  
%     mask_gbf = zeros(H,W);
...
%     G1_vec = reshape(mat2gray(G1_mat), 1, num_win);   
    ...
    if count_i == 0 %
        ...
        disp(['computing color weight time per pixel:',num2str(sum(t)/(rectIn(3)*rectIn(4)))]);
        return;
    end
...
% %                 res_i = sum(sum(weight_vec.*mask_vec))/sum(sum(weight_vec(m)));
%                 t(5,(r-1)*W+c) = toc;
...
%         disp(['count_one = ',int2str(count_one)]);
    ...
    t = toc;
    disp(['color sum time = ',num2str(t)]);
    disp(['color per pixel time = ',num2str(t/(rectIn(3)*rectIn(4)))]);
