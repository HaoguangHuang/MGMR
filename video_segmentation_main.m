% clear all;
function video_segmentation_main
    warning off;
...
%     series_mat = {'zcup_move_1','child_no1','tracking4','toy_wg_occ','dog_no_1'};
    series_mat = {'child_no1'};
    rectScale = 1.2;
    norm_box_sz = 100;
    pixel_trslation = 0; %boundingBox translation
...
%         y_mat = getY(series, idx);          %{ymap}
        ...
        [box_mat, idx, ~]= getGT_video(series);
        d_mat = getDepth_video(series, idx);
        y_mat = getY_video(series, idx);
...
function y_mat = getY_video(series, idx)
    y_fname = dir(sprintf('./videoTestData/%s/rgb/*.png',series));
    fnum = size(y_fname,1);
    y_array = cell(fnum,2); %[map,idx]
    for i = 1:fnum
        c = sscanf(y_fname(i).name,'r-%d-%d.png');
        y_fname(i).id = c(2);
        rgb = imread(sprintf('./videoTestData/%s/rgb/%s',series,y_fname(i).name));
        y = rgb2ycbcr(rgb);  
        y_array{i,1} = y;
        y_array{i,2} = y_fname(i).id;
    end
    y_array = sortrows(y_array,2);
    y_mat = y_array(idx,:); %d_mat:n*2 cell
end
...
function d_mat = getDepth_video(series, idx)
    d_fname = dir(sprintf('./videoTestData/%s/depth/*.png',series));
    fnum = size(d_fname,1);
    d_array = cell(fnum,2); %[map,idx]
    for i = 1:fnum
        d = sscanf(d_fname(i).name,'d-%d-%d.png');
        d_fname(i).id = d(2);
        depth = imread(sprintf('./videoTestData/%s/depth/%s',series,d_fname(i).name));
        depth = bitor(bitshift(depth,-3), bitshift(depth,16-3));  
        d_array{i,1} = double(depth);
        d_array{i,2} = d_fname(i).id;
    end
    d_array = sortrows(d_array,2);
    d_mat = d_array(idx,:); %d_mat:n*2 cell
...
function [box_mat, idx, gt_mat]= getGT_video(series)
    filename = sprintf('./videoTestData/%s/%s.txt',series,series);
    fid = fopen(filename);
    if fid == -1, error('open groundtruth file failed!'); end
    box_mat = [];
    while 1
        tline = fgetl(fid);
        if ~ischar(tline), break; end
        b = sscanf(tline,'%d,%d,%d,%d,%d');
        box_mat(b(5),:) = b';
    end
    fclose(fid);
    idx = box_mat(:,5);
